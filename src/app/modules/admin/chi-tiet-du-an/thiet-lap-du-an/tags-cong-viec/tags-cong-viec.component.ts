...
import { CreateTagsCongViecComponent } from './create-tagscongviec/create-tagscongviec.component';
import { EditTagsCongViecComponent } from './edit-tagscongviec/edit-tagscongviec.component';
import { TagCongViecService } from 'app/services/tagcongviec.service';


@Component({
  selector: 'app-tags-cong-viec',
  standalone: true,
  styles: [
    /* language=SCSS */
    `
  ...
  templateUrl: './tags-cong-viec.component.html'
})
export class TagsCongViecComponent {
  ...
  openCreateItemDialog(): void {
    this.dialogService.openDialog(CreateTagsCongViecComponent,
       { id: this.id },
  ...
  openEditItemDialog(data): void {
    this.dialogService.openDialog(EditTagsCongViecComponent,
       data,
       { width: '600px', height: 'auto'},
       this.getTableData.bind(this)
      )
  ...
  delData(role: any) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result === 'confirmed') {
        this._TagCongViecService.delete(role.id).subscribe(() => {
          this.getTableData();
        });
      }
    });
  ...
  // get data from api
  getTableData() {
    this.data$ = this._TagCongViecService.getNoPagingByDuAn({DuanId: this.id}).pipe(
      map((data: any) => {

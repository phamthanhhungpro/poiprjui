...
@Component({
  selector: 'app-duyet-gia-han',
  standalone: true,
  imports: [CommonModule, CommonModule, MatIconModule, MatButtonModule, ReactiveFormsModule, FormsModule],
  templateUrl: './duyet-gia-han.component.html',
})
export class DuyetGiaHanComponent {
  id: string;
  duAnSetting: any;
  duAn: any;
  congViecGroupData = [];
  ...
  getTableData(): void {
    this._congviecService.getCongViecGridByTrangThai({ duAnId: this.id, trangThai: 'CHO-DUYET-GIA-HAN' }).subscribe(res => {

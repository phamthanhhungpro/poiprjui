...
import { PerScopeService } from 'app/services/app-permission/scope.service';

@Component({
  selector: 'app-quan-ly-pham-vi',
  standalone: true,
  imports: [MatButtonModule, MatIconModule, NgIf, NgFor, MatDividerModule,
    FormsModule, MatFormFieldModule, MatInputModule, ReactiveFormsModule, MatFormFieldModule,
    MatSelectModule, MatSlideToggleModule, AsyncPipe
  ],
  templateUrl: './quan-ly-pham-vi.component.html'
})
export class QuanLyPhamViComponent {
  ...
  constructor(private _formBuilder: UntypedFormBuilder,
    ...
    private _scopeService: PerScopeService,
  ...
  save(): void {
    let data = {
      ...
      scopeIds: this.checkedItems
    };

    this._perFunctionService.assginScope(data).subscribe(res => {
      if (res.isSucceeded) {
  ...
  getFunctions() {
    ...
    this._perFunctionService.get(this.data.id).subscribe(
      res => {
        let currentEndpoint = res.scopes;
  
        this.items = this._scopeService.getAllNoPaging().pipe(

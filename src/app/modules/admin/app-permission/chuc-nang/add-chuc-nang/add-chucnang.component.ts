...
@Component({
  selector: 'app-add-chucnang',
  standalone: true,
  imports: [MatButtonModule, MatIconModule, NgIf, NgFor, MatDividerModule,
    FormsModule, MatFormFieldModule, MatInputModule, ReactiveFormsModule, MatFormFieldModule,
    MatSelectModule, SearchableSelectComponent, MatSlideToggleModule
  ],
  templateUrl: './add-chucnang.component.html'
})
export class AddChucNangComponent {
  ...
  // save data
  save(): void {
    this._perFunctionService.create(this.addFunctionForm.value).subscribe(res => {
      if (res.isSucceeded) {
